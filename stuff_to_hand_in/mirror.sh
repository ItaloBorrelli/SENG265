...
echo
	...
	then
		...
		read -p 'What would you like your new file to be called? ' filename
		echo
		...
		python2 vine_generator.py $branches $angle $scale $depth > $filename.txt
		python2 lines_to_svg_colour.py $filename.txt > $filename.svg


	elif [[ "$str" =~ ^[Ee]$ ]]
	then
		#Propt name
			read -p 'Please type the name of the line file you would like the edit: ' filename
echo
		...
		##### CHECK IF FILE EXISTS
			...
			do
				if [ -f "./$filename.txt" ]; then
					...
					read -p 'Please make sure the file is typed correctly ' filename
				fi
			done
...
read -p  'Type [Y] if you would like to mirror, otherwise press any key to continue: ' str
	...
	then
		...
		echo 'You choose to mirror the image [ '$filename' ] [' $int '] times'
		python2 mirror.py $filename.txt $int > $filename2.txt
...
echo
	if [[ "$str" =~ ^[Yy]$ ]]
		then
			...
			python2 -c1 $C1 -c2 $C2 $filename.txt > $filename2.txt
...
python2 lines_to_svg_colour.py $filename2.txt > $fileout
